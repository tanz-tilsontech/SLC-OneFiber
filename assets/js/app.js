...
function drawCharts() {
  ...
  // WPID
  $(function() {
    var result = alasql("SELECT wpid AS label, SUM(construction_footage_cx_final::NUMBER) AS footage FROM ? GROUP BY wpid", [features]);
    var columns = $.map(result, function(construction_footage_cx_final) {
